             ...
             <div className="p-4 border-b border-amber-500/30 bg-gradient-to-br from-amber-900/20 to-orange-900/10">
               ...
               <div className="space-y-2">
                 ...
                 <div className="pl-3.5 space-y-1 text-[11px] text-neutral-400">
                   <div>
                     <span className="text-neutral-500">Resistência Projetada:</span> 
                     <span className="text-red-400 font-mono ml-1">{((realPrices[selectedAsset] || 64000) * 1.015).toFixed(2)}</span>
                     <span className="text-neutral-600 ml-1">(+1.5%)</span>
                   </div>
                   <div>
                     <span className="text-neutral-500">Suporte Projetado:</span> 
                     <span className="text-emerald-400 font-mono ml-1">{((realPrices[selectedAsset] || 64000) * 0.992).toFixed(2)}</span>
                     <span className="text-neutral-600 ml-1">(-0.8%)</span>
                   </div>
                 </div>
                 
                 <div className="pt-2 mt-2 border-t border-amber-500/20">
                   <div className="text-[10px] text-amber-200 font-bold uppercase mb-1">Gatilhos Futuros:</div>
                   <div className="text-[11px] text-neutral-300 leading-relaxed">
                     <strong className="text-cyan-400">COMPRA</strong> no rompimento de {((realPrices[selectedAsset] || 64000) * 1.005).toFixed(2)} com alvo em {((realPrices[selectedAsset] || 64000) * 1.015).toFixed(2)}
                   </div>
                 </div>
               ...
               {/* 🔥 BOTÃO PARA EXPANDIR ANÁLISE COMPLETA */}
               <div className="mt-3 pt-3 border-t border-amber-500/20">
                 <button 
                   onClick={() => setShowHourlyPanel(!showHourlyPanel)}
                   className="w-full py-2 px-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20"
                 >
                   <Maximize2 className="w-3 h-3" />
                   Análise | Próxima Hora
                 </button>
               </div>
             </div>
