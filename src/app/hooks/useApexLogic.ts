    ...
    const tradingInterval = setInterval(() => {
      ...
      (async () => {
        try {
          ...
          const currentExposure = activeOrders.reduce((sum, o) => sum + o.contracts, 0); const availableLimit = Math.max(0, (aiConfig.maxContracts || 1.0) - currentExposure); const userMax = aiConfig.maxContracts || 1.0; const safetySize = Math.min(userMax, (currentBalance * 0.01) / 10); const contractsToUse = Number(aiConfig.maxContracts || 1.0);
