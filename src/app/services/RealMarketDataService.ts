 ...
 */
async function fetchBinanceDataWithRetry(symbol: string, retries = 3): Promise<RealMarketData> {
  let lastError: any;
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt < retries) {
        ...
        console.warn(`[Binance] ⚠️ Attempt ${attempt}/${retries} failed for ${symbol}. Retrying in ${delayMs}ms...`);
        await new Promise(resolve => setTimeout(resolve, delayMs));
      }
    }
  }
  
  console.error(`[Binance] ❌ All ${retries} attempts failed for ${symbol}:`, lastError.message);
  throw lastError;
}
